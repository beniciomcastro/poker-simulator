...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  await loadState();
  if (!game) await startHand(true);
  else if (game.pendingBlinds) await runHandIntroAndBlinds(true);
  botLoop();
});
...
async function startHand(showIntro = true){
  busy = true;
  clearTimeout(botTimer);
  clearTimeout(nextHandTimer);
  try {
    const data = await requestJson('api/start.php', {method:'POST'});
    game = data.game;
    lastFinishedKey = '';
...
async function runHandIntroAndBlinds(firstEntry){
  await showIntroSequence(firstEntry ? 'Partida iniciando' : 'Nova mão', firstEntry);
...
async function requestJson(url, options = {}){
  ...
  catch { throw new Error('Resposta inválida do servidor. Verifique PHP/MySQL.'); }
  if (!res.ok || data.ok === false) throw new Error(data.error || 'Erro no servidor.');
  return data;
}
...
async function botLoop(){
  clearTimeout(botTimer);
  if (!game || game.finished || game.turn === 0 || busy) { render(); return; }

  busy = true;
  renderControls();
  const p = game.players[game.turn];
  showMessage(`${p.name} pensando...`);
  await wait(DELAY.botThinkMin + Math.random() * (DELAY.botThinkMax - DELAY.botThinkMin));
...
function scheduleNextHand(){
  ...
  renderEnd();
  clearTimeout(nextHandTimer);
  if (!game.gameOver) nextHandTimer = setTimeout(() => nextHand(), DELAY.endHand);
}
...
function setupResponsiveTable(){
  ...
  window.addEventListener('orientationchange', () => setTimeout(scaleTable, 150), {passive:true});
}
...
function scaleTable(){
  ...
  document.documentElement.style.setProperty('--table-scale', String(Math.max(0.45, scale)));
}
...
function renderControls(){
  ...
  const human = game.players[0];
  const toCall = Math.max(0, (game.currentBet || 0) - (human.bet || 0));
  const isHumanTurn = !game.pendingBlinds && !game.finished && !game.gameOver && game.turn === 0 && !busy && !human.folded && !human.eliminated;
  ...
  controls.querySelectorAll('button, input').forEach(el => {
    if (!['check','call'].includes(el.dataset?.action || '')) el.disabled = !isHumanTurn;
  });
}
...
function renderPlayer(p,i){
  ...
  if (!role) { role = document.createElement('div'); role.className = 'role-badge'; seat.appendChild(role); }
...
function renderCards(container, cards, hidden, cacheName){
  if (!container) return;
  const signature = cards.map(c => c ? `${hidden ? 'X' : c.value + c.suit}` : 'empty').join('|');
  if (container.dataset.signature === signature) return;
  container.dataset.signature = signature;
  container.innerHTML = '';
  cards.forEach((c,idx) => container.appendChild(c ? cardEl(c, hidden, idx) : emptyCard()));
}

function clearCardCaches(){
  $$('.cards,.board').forEach(el => { el.dataset.signature = ''; el.innerHTML = ''; });
}
...
function cardEl(c, hidden=false, idx=0){
  ...
  el.style.animationDelay = `${idx * .12}s`;
  el.textContent = hidden ? '' : `${c.value}${c.suit}`;
  return el;
}
function emptyCard(){ const el = document.createElement('div'); el.className='card empty'; return el; }
function color(c){ return (c.suit === '♥' || c.suit === '♦') ? 'red' : 'black'; }
function stageLabel(s){ return ({preflop:'PRÉ-FLOP', flop:'FLOP', turn:'TURN', river:'RIVER', showdown:'SHOWDOWN'}[s] || s).toUpperCase(); }
...
async function animateLastAction(){
  ...
  render();
  if (['call','raise','allin'].includes(action.type)) flyChipFromSeat(action.player);
  if (action.type === 'allin') animateAllIn(action.player);
  if (action.type === 'fold') animateFold(action.player);
  const pause = ['reveal','showdown'].includes(action.type) ? DELAY.reveal : (game.finished ? 2300 : DELAY.humanAction);
  await wait(pause);
  holdEndPanel = false;
}
...
async function showIntroSequence(title, firstEntry){
  ...
  intro.querySelector('h2').textContent = firstEntry ? 'Entrando na mesa' : title;
  intro.querySelector('.intro-players').innerHTML = alive.map(p => {
    const introChips = firstEntry ? 1000 : Number(p.chips || 0);
    return `<span>${escapeHtml(p.name)}<small>${introChips} fichas</small></span>`;
  }).join('');
  intro.querySelector('p').textContent = firstEntry ? 'Preparando jogadores...' : 'Preparando próxima mão...';
...
function flyChipFromSeat(i){
  if (i < 0) return;
  const seat = $(`.seat-${i}`); const pot = $('#pot');
  if (!seat || !pot) return;
  const a = seat.getBoundingClientRect(); const b = pot.getBoundingClientRect();
  const chip = document.createElement('div'); chip.className='fly-chip';
  chip.style.left = `${a.left + a.width/2}px`; chip.style.top = `${a.top + a.height/2}px`;
  document.body.appendChild(chip);
  requestAnimationFrame(() => { chip.style.left = `${b.left + b.width/2}px`; chip.style.top = `${b.top + b.height/2}px`; chip.style.transform='scale(.55)'; });
...
function renderEnd(){
  ...
  panel.querySelector('h2').textContent = game.gameOver ? 'Fim do jogo' : (game.handResult || 'Fim da mão');

  const rows = game.players.map((p, i) => {
    const hand = (p.cards || []).map(c => miniCard(c)).join('');
    const result = p.result || (game.winners && game.winners.includes(i) ? 'Vencedor da mão' : (p.folded ? 'Fold' : 'Na mão'));
  ...
  panel.querySelector('#restartGameBtn')?.addEventListener('click', () => startHand(true));
}
...
function renderLog(){
  ...
  log.innerHTML = game.log.slice(-9).reverse().map(item => `<li>${escapeHtml(item)}</li>`).join('');
}
